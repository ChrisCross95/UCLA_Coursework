...
print_ind_blks(int blk_nmbr, int inode_number, int offset, int lvl)
{
    ...
    int ret_val = pread(fs_fd, ind_blocks,
                        sizeof(BLOCK_SIZE), block_offset);
    if (ret_val == -1) {
        fprintf(stderr, "%s\n", strerror(errno));
        exit(1);
    }
...
main(int argc, char** argv) {
    ...
    /** Get superblock data* */
    ret_val = pread(fs_fd, fs_superblk,
                    sizeof(struct ext2_super_block), SUPERBLK_OFFSET);
    if (ret_val == -1) {
        fprintf(stderr, "%s\n", strerror(errno));
        exit(1);
    }
    ...
    ret_val = pread(fs_fd, fs_blk_tbl,
                    sizeof(struct ext2_group_desc), SUPERBLK_OFFSET + BLOCK_SIZE);
    if (ret_val == -1) {
        fprintf(stderr, "%s\n", strerror(errno));
        exit(1);
    }
    ...
    ret_val = pread(fs_fd, block_bitmap, BLOCK_SIZE,
                    (fs_blk_tbl->bg_block_bitmap) * BLOCK_SIZE);
    if (ret_val == -1) {
        fprintf(stderr, "%s\n", strerror(errno));
        exit(1);
    }
    ...
    ret_val = pread(fs_fd, inode_bitmap, BLOCK_SIZE,
                    (fs_blk_tbl->bg_inode_bitmap) * BLOCK_SIZE);
    if (ret_val == -1) {
        fprintf(stderr, "%s\n", strerror(errno));
        exit(1);
    }
    ...
    ret_val = pread(fs_fd, inode_tbl_ptr, inode_table_size,
                    (fs_blk_tbl->bg_inode_table) * BLOCK_SIZE);
    if (ret_val == -1) {
        fprintf(stderr, "%s\n", strerror(errno));
        exit(1);
    }
                ...
                for (int i = 0; i < EXT2_NDIR_BLOCKS; i++)
                {
                    if (inode_curr.i_block[i] != 0)
                    {
                        ...
                        while (logical_offset < BLOCK_SIZE) {
                            ...
                            ret_val = pread(fs_fd, dir_cur,
                                            sizeof(struct ext2_dir_entry), dir_offset);
                            if (ret_val == -1) {
                                fprintf(stderr, "%s\n", strerror(errno));
                                exit(1);
                            }
