...
void insertion_sort(vector<Sensor>& s, bool comp(const Sensor&, const Sensor&))
{
    ...
    // Base case - we've reached the end of the vector
    // if (&s == &s[s.size()])
    ...
    for (int unsorted = 1; unsorted < s.size(); unsorted++) {
        ...
        // At this point, the s[loc] is where nextItem belongs
        s[loc] = nextItem; //Insert nextItem into the sorted region
    }// end insertionSort
...
int main()
{
    ...
    // destruction time.
    {
        ...
        vector <Sensor*> auxSensorPtrs(auxSensors.size());
        for (int i = 0; i < auxSensors.size(); i++)
            auxSensorPtrs[i] = &auxSensors[i];
